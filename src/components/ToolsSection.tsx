      ...
      >
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 sm:mb-12">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[#f46c38] mb-2.5">
          ...
          {capabilitiesData.map((item, index) => {
            ...
            return (
              <motion.div
                ...
                viewport={{ once: true }}
                className="group relative bg-[#141211] border border-white/10 hover:border-[#f46c38]/50 rounded-xl p-5 sm:p-6 transition-all duration-300 hover:-translate-y-1 flex flex-col justify-between shadow-md"
              >
                <div>
                  {/* Top Header: Icon & Subtle Corner Detail */}
                  <div className="flex items-center justify-between mb-4">
                    <div className="w-10 h-10 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-gray-300 group-hover:text-[#f46c38] group-hover:bg-[#f46c38]/10 group-hover:border-[#f46c38]/30 transition-all duration-300">
                      <IconComponent className="w-5 h-5 transition-transform group-hover:scale-110" />
                    </div>
                    <span className="text-[10px] font-mono text-gray-600 group-hover:text-[#f46c38]/70 transition-colors">
                      0{index + 1}
                    </span>
                  </div>

                  {/* Capability Title */}
                  <h3 className="text-lg font-bold text-white mb-1.5 group-hover:text-[#f46c38] transition-colors leading-snug">
                    {item.title[lang]}
                  </h3>
                ...
                <div className="mt-4 pt-3 border-t border-white/5 flex items-center justify-between">
                  ...
                  <span className="w-1.5 h-1.5 rounded-full bg-white/20 group-hover:bg-[#f46c38] transition-colors" />
                </div>
              </motion.div>
            );
          })}
        ...
        <div className="pt-8 border-t border-white/10">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5">
            <div className="flex items-center gap-2">
              <Wrench className="w-4 h-4 text-[#f46c38]" />
            ...
            {toolkitData.map((tool, idx) => (
              <div
                ...
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs sm:text-sm font-medium border transition-all duration-200 cursor-default ${
                  tool.isAi
                    ? 'bg-[#f46c38]/10 text-[#f46c38] border-[#f46c38]/30 hover:bg-[#f46c38]/20'
